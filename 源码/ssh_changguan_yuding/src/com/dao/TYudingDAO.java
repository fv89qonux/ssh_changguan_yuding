...
public class TYudingDAO extends HibernateDaoSupport
{
	...
	public static final String QCH_ID = "qchId";
    ...
    public List findByPropertys(Integer userId, String ydrq)
    {
        log.debug("finding TYuding instance with property: ");
        try
        {
            String queryString = "from TYuding  where    del='no' and    userId='" + userId + "' AND  ydrq='" + ydrq + "'";
            List<TYuding> list = getHibernateTemplate().find(queryString);
            return list;
        } catch (RuntimeException re)
        {
            log.error("find by property name failed", re);
	...
	public List findByQchId(Object qchId)
	{
		return findByProperty(QCH_ID, qchId);
	}
