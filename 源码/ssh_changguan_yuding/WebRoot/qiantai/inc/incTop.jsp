...
<html>
  <head>
     <style type="text/css">
        .Header {background: url(<%=path %>/img/banner.jpg) #d10e00 repeat-x left top; height: 193px;width: 966px;}
     ...
     <script type="text/javascript">
         ...
         function myzl(){
	            ...
	            <c:if test="${sessionScope.userType==1}">
	                window.location.href="<%=path %>/myYudingMana.action";
	            </c:if>
         }         
     </script>
  </head>
  ...
  <body>
        <div class="Header HeaderTop">
			<br/>
			<font style="font-size: 40px;color: blue;font-weight: bolder;display: block;text-align: center;">体育馆场地预约系统</font>
		</div>
		<div class="topmenu cbody1">
			<ul>
				...
				<li class="thisclass">
					<A href="<%=path %>/qchByCatelog.action?catelogId=${catelog.catelogId}">${catelog.catelogName}</A>
				</li>    
				</c:forEach>
				<li class="thisclass"><A href="#" onclick="reg()">在线注册</A></li>
				<li class="thisclass"><A href="#" onclick="myxinxi()">我的信息</A></li>
				<li class="thisclass"><A href="#" onclick="myzl()">我的预订</A></li>
				<li class="thisclass"><A href="#" onclick="liuyanAll()">留言板</A></li>
				<li class="thisclass"><a target="_blank" href="<%=path %>/login.jsp">管理登录</a></li>				
			</ul>
		</div>
		<form id="searchForm" action="<%=path %>/qchSearch.action" method="post">
			<div class="topsearch">
				<div class="title"></div>
